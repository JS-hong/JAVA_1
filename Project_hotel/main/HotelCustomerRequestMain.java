...
//		job="ADM";
	...
	public void menu(String sessioncode,String sessionjob) { //고객요청메뉴
		...
		case 3: // 3 선택하면
			//if문 넣기, "ADM"이면, menuadmin(), "EMP"이면, menuemp()
			if (job.equals("ADM")) {
				hotelmain.menuadmin();
...
//		고객요청사항의 카테고리를 입력받고, 해당 카테고리의 고객요청사항을 조회한다.
		...
		if (reqvoList.size() > 0) { //등록된 카테고리가 있으면 화면에 표시
			...
			for (HotelCustomerRequestVO reqvo : reqvoList) { // reqvoList에서 reqvo값이 없을 때까지 출력
	...
	public void modify() { //고객요청사항 수정
		...
		System.out.print("수정 할 고객요청사항의 등록번호를 입력하세요: ");
		num = sc.nextInt();
		System.out.println("----------------------------");
		...
		System.out.print("수정 할 카테고리 항목을 선택하세요: ");
		int category = sc.nextInt();
		System.out.println();
		...
		System.out.print("수정 할 요청사항을 입력하세요: ");
		reqMessage = sc.next();
	...
	public void remove() { //고객요청사항 삭제
		...
		System.out.print("삭제 할 고객요청사항의 등록번호를 입력하세요: ");
		num = sc.nextInt();
		reqvo.setNum(num);
		System.out.println("----------------------------");
		...
		if(input.equalsIgnoreCase("Y")) {
			...
			boolean result = reqdao.delete(num);
