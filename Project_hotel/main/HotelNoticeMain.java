...
//		EMP_003	공지사항 전체목록 화면 (직원용)
		...
		if (noticevoList.size() > 0) { // 등록된 공지사항이 있으면 화면에 표시
			...
			System.out.print(" 등록번호 | 제목 | 작성자 | 작성일 ");
...
//		COM_012	공지사항 상세보기 (직원용)
		...
		if (noticevo != null) {
			...
			System.out.println("작성자: " + noticevo.getEcode());
...
//		ADM_013	공지사항 상세보기 화면(관리자용)
		...
		if (noticevo != null) {
			...
			case 3: // 3 선택하면
				listadm(); // ADM_012 공지사항 전체목록 화면(관리자용)
				break;
			default: // 그 외의 경우에는 "1~3 중에 하나를 입력해주세요."를 출력하고
						// >>선택 : "을 출력
...
//		ADM_015	공지사항 수정화면
		...
		System.out.println("제목: ");
		noticevo.setTitle(sc.next());

		System.out.println("수정 할 내용: ");
		noticevo.setContent(sc.next());
		System.out.println("----------------------------");
